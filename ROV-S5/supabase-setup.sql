-- ─────────────────────────────────────────────────────────────────────────────
--  ROV — Supabase Database Setup
--  Run this entire file in: Supabase Dashboard → SQL Editor → New query
-- ─────────────────────────────────────────────────────────────────────────────


-- 1. PROFILES TABLE
--    Links each auth user to a role (admin or client) and optionally a client_id
-- ─────────────────────────────────────────────────────────────────────────────
create table if not exists profiles (
  id         uuid references auth.users(id) on delete cascade primary key,
  role       text not null check (role in ('admin', 'client')),
  client_id  text,  -- e.g. 'hhzero' — null for admin
...
-- Only admins can insert/update profiles (handled via service role in admin panel)
...
create table if not exists clients (
  id                    text primary key,  -- e.g. 'hhzero'
  name                  text not null,
  short_name            text,
  contact               text,
  email                 text,
  ...
  weight_per_case       int    default 25,
...
create table if not exists skus (
  id              text primary key,  -- e.g. 'HHZ-001'
...
create policy "Clients can read own skus"
  ...
  using (
    exists (
      select 1 from profiles
      where id = auth.uid() and client_id = skus.client_id
    )
  );
...
create table if not exists orders (
  id               text primary key,  -- e.g. 'ORD-2051'
...
-- 5. SEED: HH Zero client data
-- ─────────────────────────────────────────────────────────────────────────────
insert into clients (
  id, name, short_name, contact, email,
  fulfillment_fee, billing_cycle_days, payment_method,
  storage_start, storage_free_end, storage_billing_start,
  storage_cost_per_pallet, cases_per_pallet, weight_per_case, status
) values (
  'hhzero', 'HH Zero', 'HHZ', 'Denisa', 'denisa@hhzero.com',
  3.00, 15, 'Zelle / ACH',
  '2026-04-20', '2026-06-19', '2026-06-20',
  20.00, 70, 25, 'active'
